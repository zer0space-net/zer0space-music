...
async def api_import_backup(request: Request) -> dict[str, Any]:
    """Restores playlists from a file this app's own export produced
    (library.EXPORT_FORMAT_PLAYLIST/_LIBRARY) — already carries our
    catalogue key per track, so this is a straight create+add, no matching.
    For a plain title/artist list with no key at all, see
    POST /api/playlists/import-csv instead."""
    user = me(request)
    payload = await backup_body(request)
    return await library.import_backup(user, payload)


# Column names to look for in a header row, most specific first — checked as
# a whole shape (a title/artist/duration match all from the same shape)
# rather than mixing candidates from different tools, which is how a stray
# "Name" column would otherwise grab the wrong field. Exportify
# (exportify.net, a common way to get a Spotify playlist to CSV at all) is
# the first shape — its real header is exactly
# "Track URI,Track Name,Album Name,Artist Name(s),Release Date,Duration (ms),...":
# naive column-0/column-1 reads the URI as the title and finds nothing for
# every single row, which is the bug this shape exists to fix. Its duration
# column is also worth reading: duration is the strongest signal
# playlist_import._score has, and Exportify happens to hand it over for
# free where a hand-written or LLM-generated list never has it. The second
# shape is the plain two-column "title,artist" this format was designed
# around, English or German — no duration column to look for there, the
# format has nowhere unambiguous to put one.
_CSV_HEADER_SHAPES: list[tuple[set[str], set[str], set[str]]] = [
    ({"track name"}, {"artist name(s)", "artist name", "artist names"}, {"duration (ms)", "duration_ms"}),
    ({"title", "song", "song name", "track", "titel"}, {"artist", "artists", "künstler", "interpret"}, set()),
]


def _find_csv_columns(header: list[str]) -> tuple[int, int | None, int | None] | None:
    """The (title, artist, duration_ms) column indices for a recognised
    header shape, or ``None`` if this looks like a headerless file — see
    _parse_csv_tracks. ``artist`` and ``duration_ms`` are each optional even
    within a matched shape."""
    lowered = [cell.strip().lower() for cell in header]
    for title_names, artist_names, duration_names in _CSV_HEADER_SHAPES:
        title_idx = next((i for i, cell in enumerate(lowered) if cell in title_names), None)
        if title_idx is None:
            continue
        artist_idx = next((i for i, cell in enumerate(lowered) if cell in artist_names), None)
        duration_idx = next((i for i, cell in enumerate(lowered) if cell in duration_names), None)
        return title_idx, artist_idx, duration_idx
    return None


def _parse_csv_tracks(text: str) -> list[dict[str, Any]]:
    """Parses a CSV into title/artist(/duration) rows — the low-ceremony
    alternative to a JSON tracklist, meant to be hand-typed, exported from a
    tool like Exportify, or generated by an LLM without having to get JSON
    syntax right. A row with no title is skipped, not fatal; a row with no
    artist still gets a title-only search (see playlist_import._best_match)
    — the scorer works fine without one, just with a smaller signal.
    """
    rows = [row for row in csv.reader(io.StringIO(text)) if any(cell.strip() for cell in row)]
    if not rows:
        return []

    columns = _find_csv_columns(rows[0])
    if columns is not None:
        title_idx, artist_idx, duration_idx = columns
        data_rows = rows[1:]
    else:
        # No recognised header — assume the simplest shape, column 0 is the
        # title and column 1 (if present) is the artist. No duration column
        # to guess at here: a bare number could be seconds or milliseconds
        # with no header to say which, and a wrong guess is worse than none.
        title_idx, artist_idx, duration_idx = 0, (1 if len(rows[0]) >= 2 else None), None
        data_rows = rows
    ...
    for row in data_rows:
        ...
        if raw_duration_ms.isdigit():
            entry["duration"] = round(int(raw_duration_ms) / 1000)
        tracks.append(entry)
    return tracks


@app.post("/api/playlists/import-csv", include_in_schema=False)
async def api_import_csv(request: Request) -> dict[str, Any]:
    """A CSV file, one song per line ("title,artist" — artist optional, a
    header row is recognised and skipped). Never touches Spotify's page at
    all, so it is not subject to whatever cap that page imposes; goes
    through the same Deezer matching pass as the Spotify import either way.
    The playlist name comes from ?name=, which the client fills in from the
    uploaded file's name."""
    user = me(request)
    raw = await request.body()
    if len(raw) > MAX_BACKUP_BODY:
        raise ApiError(413, "BODY_TOO_LARGE", "CSV file too large")
    try:
        text = raw.decode("utf-8-sig")  # -sig: strips a BOM some spreadsheet exports add
    except UnicodeDecodeError:
        raise ApiError(400, "BAD_CSV", "The file is not valid UTF-8 text") from None

    name = (request.query_params.get("name") or "Imported playlist")[:120]
    tracks = _parse_csv_tracks(text)
    return await playlist_import.import_track_list(user, {"name": name, "tracks": tracks})
