...
# Two header spellings recognised as "skip this row" — English and German,
# since the file this exists for is exactly as likely to be typed by a
# German-speaking user as pasted together by an LLM in English.
_CSV_HEADER_TITLES = {"title", "song", "track", "titel"}


def _parse_csv_tracks(text: str) -> list[dict[str, str]]:
    """Parses a "title,artist" per line CSV — the low-ceremony alternative
    to a JSON tracklist, meant to be hand-typed or generated by an LLM
    without having to get JSON syntax right. A row is skipped, not fatal, if
    it has no title; the artist column is optional (a title-only search
    still works, just with a smaller signal — see playlist_import._best_match).
    """
    rows = [row for row in csv.reader(io.StringIO(text)) if any(cell.strip() for cell in row)]
    if rows and rows[0] and rows[0][0].strip().lower() in _CSV_HEADER_TITLES:
        rows = rows[1:]
    tracks = []
    for row in rows:
        ...
        if not title:
            continue
        tracks.append({"title": title, "artist": (row[1] if len(row) > 1 else "").strip()})
    return tracks
