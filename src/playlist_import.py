...
# all this needs per song, so a big playlist can be typed by hand or
# generated by an LLM asked to list a playlist's tracks — no scraping, no
# Spotify involved at all, and no cap but MAX_TRACKLIST_ENTRIES. Reuses the
# exact same matching pass as the Spotify import; only where the list of
# titles comes from differs.

EXPORT_FORMAT_TRACKLIST = "zer0space-music-tracklist"
